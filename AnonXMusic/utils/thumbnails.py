...
async def get_thumb(videoid, user_id):
    ...
    try:
        ...
        y=changeImageSize(360,360,circle(youtube)) 
        background.paste(y,(810,130),mask=y)
        a=changeImageSize(130,130,circle(xp)) 
        background.paste(a,(1050,380),mask=a)
        draw = ImageDraw.Draw(background)
        arial = ImageFont.truetype("AnonXMusic/assets/font2.ttf", 30)
        font = ImageFont.truetype("AnonXMusic/assets/font.ttf", 30)
        draw.text((1105, 10), unidecode(app.name), fill="white", font=arial)
        draw.text(
                (55, 560),
                f"{channel} | {views[:23]}",
                (255, 255, 255),
